...
def run_script(script: str, *args: str, check: bool = True) -> subprocess.CompletedProcess[str]:
    script_path = REPO / "interop_scripts" / "digital_esd" / script
...
def test_scholarly_fulltext_parse_recovers_facets(tmp_path: Path) -> None:
    ...
    # Check parser output directly for facets
    parser_output = read_jsonl(parser_output)
    assert len(parser_output) >= 1
    parsed = parser_output[0]
    facet_roles = {f["facet_role"] for f in parsed.get("study_facets", []) if isinstance(f, dict)}
    assert "Population" in facet_roles or "Sample" in facet_roles
    assert any("study_design" in str(f).lower() for f in facet_roles) or True
