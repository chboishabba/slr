...
fn live_mabo_world_reports_scale_frontier_and_non_authority() {
    ...
    assert!(world.deepest_observed_hop <= world.requested_max_hops);
    assert!(world.visited_refs.len() <= 50_000);
    assert!(world.edges.len() <= 100_000);

    let unique_visited: BTreeSet<&str> = world.visited_refs.iter().map(String::as_str).collect();
    assert_eq!(unique_visited.len(), world.visited_refs.len());
