        ...
        match xml.read_event_into(&mut buffer)? {
            Event::Start(start) => {
                match start.name().as_ref() {
                    b"rdf:Description" => {
                        current_entity = attribute_value(&start, b"rdf:about")?
            ...
            Event::Empty(empty) => {
                ...
                if let Some(property) = std::str::from_utf8(raw).ok().and_then(|value| value.strip_prefix("wdt:")) {
                    ...
                    article_about_root = attribute_value(&empty, b"rdf:resource")?
                        .and_then(|value| qid_from_entity_uri(&value))
                        .as_deref() == Some(root_qid);
                } else if raw == b"schema:isPartOf" && current_article_url.is_some() {
                    article_is_enwiki = attribute_value(&empty, b"rdf:resource")?
                        .as_deref() == Some(ENWIKI_ROOT);
                }
            }
            Event::End(end) => {
                match end.name().as_ref() {
                    b"rdf:Description" => current_entity = None,
                    b"schema:Article" => {
