use sensiblaw_wikimedia_candidate_provider::{emit_candidates_from_rdf, fetch_and_emit};
use std::env;
use std::fs::File;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match command {
        "fetch" => {
            ...
            let receipt = fetch_and_emit(&qid, &mut writer)?;
            writer.flush()?;
        ...
        _ => {
            eprintln!("usage: sensiblaw-wikimedia-candidate-provider fetch --qid QID --output candidates.slrg");
