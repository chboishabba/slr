...
fn main() {
    let source_identity = "case:Cullen-v-State-of-Queensland-2026-HCA-19";
    let proposition = "prop:cullen-positive-operational-duty";
    let citation = "[2026] HCA 19";
    let explicit = deterministic_mnc_to_austlii(citation).expect("known HCA MNC must lower");
    ...
    let transport = FixtureTransport {
        response: Some(HttpResponse {
            final_url: explicit.clone(),
            status_code: 200,
            content_type: Some("text/html".into()),
            body: b"<html><h1>Cullen fixture</h1></html>".to_vec(),
        }),
    };
    ...
    let fetched = executor.fetch_austlii(&explicit).expect("bounded fixture fetch");
    ...
    let ingested = mark_locally_ingested(
        &fetched,
        source_identity,
        "source:cullen:rev:fixture-1",
        "sha256:cullen-fixture",
    );
    ...
    println!(
        "first_network_requests=1 replay_network_requests=0 source={} proposition={} authority={}",
