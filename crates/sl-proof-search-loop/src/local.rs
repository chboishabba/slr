...
impl LocalIndex {
    ...
    pub fn search(&self, expr: &QueryExpr) -> Vec<CandidatePassage> {
        self.docs.values().filter(|doc| matches_expr(doc, expr)).map(|doc| CandidatePassage {
...
fn positions(tokens: &[String], needle: &str) -> Vec<usize> {
    ...
    tokens.iter().enumerate().filter_map(|(i,t)| (t == &n).then_some(i)).collect()
}

fn phrase_tokens(s: &str) -> Vec<String> { tokenize(s) }

fn phrase_positions(tokens: &[String], phrase: &str) -> Vec<usize> {
    let p = phrase_tokens(phrase);
    if p.is_empty() || p.len() > tokens.len() { return Vec::new(); }
    tokens.windows(p.len()).enumerate().filter_map(|(i,w)| (w == p.as_slice()).then_some(i)).collect()
}
...
fn proximity(doc: &LocalDocument, left: &QueryExpr, right: &QueryExpr, words: u32, ordered: bool) -> bool {
    ...
    let l = locs(left);
    let r = locs(right);
    l.iter().any(|a| r.iter().any(|b| {
        if ordered { *a <= *b && (*b - *a) <= words as usize }
        else { a.abs_diff(*b) <= words as usize }
    }))
}
...
fn matches_expr(doc: &LocalDocument, expr: &QueryExpr) -> bool {
    ...
    match expr {
        ...
        QueryExpr::DateRange { start, end } => doc.date_ref.as_ref().is_some_and(|d| d >= start && d <= end),
    }
}
...
mod tests {
    ...
    fn local_proximity_and_citation_search_are_network_free_by_construction() {
        let q = QueryExpr::Near { left: Box::new(QueryExpr::Term("operational".into())), right: Box::new(QueryExpr::Term("risk".into())), words: 5 };
        assert_eq!(index().search(&q).len(), 1);
        assert_eq!(index().search(&QueryExpr::Citation("Donoghue v Stevenson".into())).len(), 1);
    }
}
